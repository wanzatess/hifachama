...
__export(react_router_exports, {
  ...
  UNSAFE_SingleFetchNavigateSymbol: () => SingleFetchNavigateSymbol,
...
var NavigateDocument = (url, init) => {
  let response = Navigate(url, init);
...
var NavigateStatusCodes = /* @__PURE__ */ new Set([301, 302, 303, 307, 308]);
var NavigatePreserveMethodStatusCodes = /* @__PURE__ */ new Set([307, 308]);
var IDLE_NAVIGATION = {
  state: "idle",
  location: void 0,
  formMethod: void 0,
...
function createRouter(init) {
  ...
  let fetchNavigateIds = /* @__PURE__ */ new Set();
  ...
  function completeNavigation(location, newState, { flushSync } = {}) {
    let isActionReload = state.actionData != null && state.navigation.formMethod != null && isMutationMethod(state.navigation.formMethod) && state.navigation.state === "loading" && location.state?._isNavigate !== true;
    let actionData;
    ...
    let preventScrollReset = pendingPreventScrollReset === true || state.navigation.formMethod != null && isMutationMethod(state.navigation.formMethod) && location.state?._isNavigate !== true;
  ...
  async function handleAction(request, location, submission, matches, scopedContext, isFogOfWar, opts = {}) {
    ...
    if (isNavigateResult(result)) {
      ...
      if (opts && opts.replace != null) {
        ...
        let location2 = normalizeNavigateLocation(
      ...
      await startNavigateNavigation(request, result, true, {
        submission,
        replace: replace2
      });
      return { shortCircuited: true };
  ...
  async function handleLoaders(request, location, matches, scopedContext, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace2, initialHydration, flushSync, pendingActionResult) {
    ...
    if (matchesToLoad.length === 0 && revalidatingFetchers.length === 0) {
      let updatedFetchers2 = markFetchNavigatesDone();
      completeNavigation(
        location,
        {
          matches,
    ...
    let Navigate2 = findNavigate(loaderResults);
    if (Navigate2) {
      await startNavigateNavigation(request, Navigate2.result, true, {
        replace: replace2
      });
      return { shortCircuited: true };
    }
    Navigate2 = findNavigate(fetcherResults);
    if (Navigate2) {
      fetchNavigateIds.add(Navigate2.key);
      await startNavigateNavigation(request, Navigate2.result, true, {
        replace: replace2
      });
      return { shortCircuited: true };
    ...
    let updatedFetchers = markFetchNavigatesDone();
  ...
  async function handleFetcherAction(key, routeId, path, match, requestMatches, scopedContext, isFogOfWar, flushSync, preventScrollReset, submission) {
    ...
    if (fetchersQueuedForDeletion.has(key)) {
      if (isNavigateResult(actionResult) || isErrorResult(actionResult)) {
        updateFetcherState(key, getDoneFetcher(void 0));
        return;
      }
    } else {
      if (isNavigateResult(actionResult)) {
        fetchControllers.delete(key);
        if (pendingNavigationLoadId > originatingLoadId) {
          updateFetcherState(key, getDoneFetcher(void 0));
          return;
        } else {
          fetchNavigateIds.add(key);
          updateFetcherState(key, getLoadingFetcher(submission));
          return startNavigateNavigation(fetchRequest, actionResult, false, {
    ...
    let Navigate2 = findNavigate(loaderResults);
    if (Navigate2) {
      return startNavigateNavigation(
        revalidationRequest,
        Navigate2.result,
        false,
        { preventScrollReset }
  ...
  async function handleFetcherLoader(key, routeId, path, match, matches, scopedContext, isFogOfWar, flushSync, preventScrollReset, submission) {
    ...
    if (isNavigateResult(result)) {
      if (pendingNavigationLoadId > originatingLoadId) {
        ...
        fetchNavigateIds.add(key);
        await startNavigateNavigation(fetchRequest, result, false, {
          preventScrollReset
        });
        return;
      }
  ...
  async function startNavigateNavigation(request, Navigate2, isNavigation, {
    submission,
    fetcherSubmission,
    preventScrollReset,
    replace: replace2
  } = {}) {
    if (Navigate2.response.headers.has("X-Remix-Revalidate")) {
      isRevalidationRequired = true;
    }
    let location = Navigate2.response.headers.get("Location");
    invariant(location, "Expected a Location header on the Navigate Response");
    location = normalizeNavigateLocation(
      location,
      new URL(request.url),
      basename
    );
    let NavigateLocation = createLocation(state.location, location, {
      _isNavigate: true
    });
    if (isBrowser2) {
      let isDocumentReload = false;
      if (Navigate2.response.headers.has("X-Remix-Reload-Document")) {
    ...
    pendingNavigationController = null;
    let NavigateNavigationType = replace2 === true || Navigate2.response.headers.has("X-Remix-Replace") ? "REPLACE" /* Replace */ : "PUSH" /* Push */;
    let { formMethod, formAction, formEncType } = state.navigation;
    if (!submission && !fetcherSubmission && formMethod && formAction && formEncType) {
      submission = getSubmissionFromNavigation(state.navigation);
    }
    let activeSubmission = submission || fetcherSubmission;
    if (NavigatePreserveMethodStatusCodes.has(Navigate2.response.status) && activeSubmission && isMutationMethod(activeSubmission.formMethod)) {
      await startNavigation(NavigateNavigationType, NavigateLocation, {
        submission: {
          ...activeSubmission,
          formAction: location
        },
        // Preserve these flags across Navigates
  ...
  async function callDataStrategy(type, request, matchesToLoad, matches, scopedContext, fetcherKey) {
    ...
    for (let [routeId, result] of Object.entries(results)) {
      if (isNavigateDataStrategyResult(result)) {
        let response = result.result;
        dataResults[routeId] = {
          type: "Navigate" /* Navigate */,
          response: normalizeRelativeRoutingNavigateResponse(
            response,
            request,
            routeId,
            matches,
  ...
  function markFetchNavigatesDone() {
    let doneKeys = [];
    let updatedFetchers = false;
    for (let key of fetchNavigateIds) {
      let fetcher = state.fetchers.get(key);
  ...
  async function callDataStrategy(type, request, matchesToLoad, matches, isRouteRequest, requestContext, dataStrategy) {
    ...
    await Promise.all(
      matches.map(async (match) => {
        ...
        if (isNavigateDataStrategyResult(result)) {
          let response = result.result;
          throw normalizeRelativeRoutingNavigateResponse(
            response,
            request,
            match.route.id,
            matches,
...
function getMatchesToLoad(history, state, matches, submission, location, initialHydration, isRevalidationRequired, cancelledFetcherLoads, fetchersQueuedForDeletion, fetchLoadMatches, fetchNavigateIds, routesToUse, basename, pendingActionResult) {
...
function normalizeRelativeRoutingNavigateResponse(response, request, routeId, matches, basename) {
  let location = response.headers.get("Location");
  invariant(
    location,
    "Navigates returned/thrown from loaders/actions must have a Location header"
  );
...
function normalizeNavigateLocation(location, currentUrl, basename) {
  if (ABSOLUTE_URL_REGEX.test(location)) {
...
function processRouteLoaderData(matches, results, pendingActionResult, isStaticHandler = false, skipLoaderErrorBubbling = false) {
  ...
  matches.forEach((match) => {
    ...
    invariant(
      !isNavigateResult(result),
      "Cannot handle Navigate results in processLoaderData"
    );
    if (isErrorResult(result)) {
...
function processLoaderData(state, matches, results, pendingActionResult, revalidatingFetchers, fetcherResults) {
  ...
  revalidatingFetchers.forEach((rf) => {
    ...
    if (controller && controller.signal.aborted) {
      ...
      state.fetchers.delete(key);
    } else if (isNavigateResult(result)) {
      invariant(false, "Unhandled fetcher revalidation Navigate");
    } else {
...
function isNavigateDataStrategyResult(result) {
  return isResponse(result.result) && NavigateStatusCodes.has(result.result.status);
}
function isErrorResult(result) {
  return result.type === "error" /* error */;
}
function isNavigateResult(result) {
  return (result && result.type) === "Navigate" /* Navigate */;
}
...
function isNavigateStatusCode(statusCode) {
  return NavigateStatusCodes.has(statusCode);
}
function isNavigateResponse(result) {
  return isResponse(result) && isNavigateStatusCode(result.status) && result.headers.has("Location");
}
...
// lib/dom/ssr/single-fetch.tsx
var SingleFetchNavigateSymbol = Symbol("SingleFetchNavigate");
  ...
  return (0, import_turbo_stream.decode)(body, {
    plugins: [
      (type, ...rest) => {
        ...
        if (type === "SingleFetchNavigate") {
          return { value: { [SingleFetchNavigateSymbol]: rest[0] } };
        }
...
function unwrapSingleFetchResults(results, routeId) {
  let Navigate2 = results[SingleFetchNavigateSymbol];
  if (Navigate2) {
    return unwrapSingleFetchResult(Navigate2, routeId);
  }
...
function unwrapSingleFetchResult(result, routeId) {
  if ("error" in result) {
    ...
    throw Navigate(result.Navigate, { status: result.status, headers });
...
async function callRouteHandler(handler, args) {
  ...
  if (isDataWithResponseInit(result) && result.init && result.init.status && isNavigateStatusCode(result.init.status)) {
...
var SINGLE_FETCH_Navigate_STATUS = 202;
async function singleFetchAction(build, serverMode, staticHandler, request, handlerUrl, loadContext, handleError) {
  try {
    let respond2 = function(context) {
      let headers = getDocumentHeaders(build, context);
      if (isNavigateStatusCode(context.statusCode) && headers.has("Location")) {
        return generateSingleFetchResponse(request, build, serverMode, {
          result: getSingleFetchNavigate(
            context.statusCode,
            headers,
            build.basename
          ),
          headers,
          status: SINGLE_FETCH_Navigate_STATUS
        });
      }
      if (context.errors) {
  ...
  try {
    let respond2 = function(context) {
      ...
      if (isNavigateStatusCode(context.statusCode) && headers.has("Location")) {
        return generateSingleFetchResponse(request, build, serverMode, {
          result: {
            [SingleFetchNavigateSymbol]: getSingleFetchNavigate(
              context.statusCode,
              headers,
              build.basename
            )
...
function getSingleFetchNavigate(status, headers, basename) {
  let Navigate2 = headers.get("Location");
  if (basename) {
    Navigate2 = stripBasename(Navigate2, basename) || Navigate2;
  }
  return {
    Navigate: Navigate2,
    status,
  ...
  return (0, import_turbo_stream2.encode)(data2, {
    ...
    plugins: [
      (value) => {
        ...
        if (value && typeof value === "object" && SingleFetchNavigateSymbol in value) {
          return ["SingleFetchNavigate", value[SingleFetchNavigateSymbol]];
        }
      }
    ],
    postPlugins: [
  ...
  return async function requestHandler(request, initialContext) {
    ...
    if (url.pathname.endsWith(".data")) {
      ...
      if (_build.entry.module.handleDataRequest) {
        ...
        if (isNavigateResponse(response)) {
          let result = getSingleFetchNavigate(
            response.status,
            response.headers,
            _build.basename
          );
          if (request.method === "GET") {
            result = {
              [SingleFetchNavigateSymbol]: result
            };
          }
...
0 && (module.exports = {
  ...
  UNSAFE_RouteContext,
  UNSAFE_ServerMode,
  UNSAFE_SingleFetchNavigateSymbol,
